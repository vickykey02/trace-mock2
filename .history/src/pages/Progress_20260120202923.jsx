...
const ALL_BADGES = [
  ...
  {/*
    ...
    name: 'Wissenssammler',
	levels: [
		{ level: 1, count: 5, name: 'Bronze', icon: '🥉', min: 2, color: 'cd7f32' },
		{ level: 2, count: 15, name: 'Silber', icon: '🥈' },
		{ level: 3, count: 30, name: 'Gold', icon: '🥇' },
	],
    icon: '📚',
  ...
  */},
  {
    ...
    name: 'Wissenssammler',
    levels: [
      { level: 'Bronze', icon: '🥉', min: 2, color: '#cd7f32' },
      { level: 'Silber', icon: '🥈', min: 4, color: '#c0c0c0' },
      { level: 'Gold', icon: '🥇', min: 5, color: '#ffd700' },
    ],
    description: 'Sammle Tipps und erreiche verschiedene Stufen!',
    getLevel: (count) => {
      if (count >= 5) return 2;
      if (count >= 4) return 1;
      if (count >= 2) return 0;
