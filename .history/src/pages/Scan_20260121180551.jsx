...
const Scan = () => {
    ...
    const scanInterval = setInterval(() => {
      if (videoRef.current && canvasRef.current && isScanning) {
        try {
          ...
          // Für eine echte Implementierung würde man jsQR oder ähnliches verwenden
  ...
  const detectQRCode = (imageData) => {
    ...
    // In Produktion würde man jsQR (https://github.com/cozmo/jsQR) verwenden
    // Für jetzt: erkenne dunkle Muster die QR-Codes ähneln
    ...
    if (darkRatio > 0.2 && darkRatio < 0.8) {
      // Hier würde die echte QR-Dekodierung stattfinden
      // Für Demo: Simuliere einen gescannten QR-Code
      simulateQRScan();
    }
  };
  ...
  const simulateQRScan = () => {
    ...
    // Starte nur einmal bei erkanntem Code
    if (!showPopup) {
