...
//noch fehlen etc.) -> auch Einlösebedingungen mit rein
...
// Auskommentierte Beispiel-Belohnungen für Referenz
{/*const ALL_REWARDS = [
  {
    id: 1,
...
const RewardCard = ({ reward, currentBonusPoints, onRedeem, isRedeemed }) => {
  const navigate = useNavigate();
  const canRedeem = currentBonusPoints >= reward.pointsCost && !isRedeemed;
  const progress = Math.min(100, Math.round((currentBonusPoints / reward.pointsCost) * 100));
  const pointsNeeded = Math.max(0, reward.pointsCost - currentBonusPoints);
  const [isSpendOpen, setIsSpendOpen] = useState(false);

  const handleCardClick = () => {
    navigate(`/reward-detail/${reward.id}`);
  };
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            ...
            <>
              ...
              <p style={{ margin: 0, fontSize: 12, color: canRedeem ? '#128b09' : '#999' }}>
                {canRedeem 
                  ...
                  : `${currentBonusPoints} / ${reward.pointsCost} Punkte (noch ${pointsNeeded} nötig)`
                }
              </p>
            </>
          )}
        ...
        {canRedeem && !isRedeemed && (
          <button
            onClick={(e) => {
              e.stopPropagation(); // Verhindert Navigation zur Detail-Seite
              setIsSpendOpen(true);
            }}
            style={{
              background: '#128b09',
...
export default function Rewards() {
  const { rewards: ALL_REWARDS } = useRewards();
  const { bonusPoints, subtractBonusPoints } = useBonusPoints();
  const [redeemedIds, setRedeemedIds] = useState([]);

  const handleRedeem = (reward) => {
    if (bonusPoints >= reward.pointsCost) {
      subtractBonusPoints(reward.pointsCost, `Belohnung eingelöst: ${reward.title}`);
  ...
  const sortedRewards = [...ALL_REWARDS].sort((a, b) => {
    const aCanRedeem = bonusPoints >= a.pointsCost && !redeemedIds.includes(a.id);
    const bCanRedeem = bonusPoints >= b.pointsCost && !redeemedIds.includes(b.id);
  ...
  const redeemableCount = ALL_REWARDS.filter(r => 
    bonusPoints >= r.pointsCost && !redeemedIds.includes(r.id)
  ).length;
  ...
  return (
    <div style={{ padding: '30px', paddingBottom: '100px' }}>
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: '14px', opacity: 0.9 }}>Deine Bonuspunkte</div>
          <div style={{ fontSize: '28px', fontWeight: 'bold' }}>{bonusPoints}</div>
        </div>
        {redeemableCount > 0 && (
          <div style={{
            backgroundColor: '#fff',
