...
const ALL_BADGES = [
  {
    ...
    criteria: '1 Handlung hinzufügen',
    check: (ctx) => ctx.approvedCount >= 1,  // ctx.approvedCount statt remembered
  },
  {
    id: 2,
    name: 'Erster Tipp',
    ...
    check: (ctx) => ctx.remembered.length >= 1,
  },
  {
    id: 3,
    name: 'Wissenssammler',
  ...
  },
  {
    ...
    check: (ctx) => ctx.remembered.length >= 10,
  },
];
...
export default function Progress() {
  ...
  const approvedCount = submitted?.filter(a => a.status === 'approved').length || 0;

  // Context-Objekt für Badge-Checks
  const ctx = { remembered, approvedCount };
  ...
  const nextBadge = ALL_BADGES.find(b => !b.levels && !b.check?.(ctx));
  ...
  return (
    <div style={{ padding: 30 }}>
      <PageHeader title="Deine Erfolge" />
