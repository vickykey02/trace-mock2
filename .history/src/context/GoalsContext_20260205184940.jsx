...
export const PRESET_GOALS = [
  {
    id: 'preset-1',
    title: '100kg CO2 einsparen innerhalb einer Woche',
    description: 'Spart gemeinsam 100kg CO2 in 7 Tagen',
    category: 'CO2 & Wasser',
    label: 'Community',
    target: 100,
    unit: 'Kilogramm',
    linkedAction: ['Bus/Bahn fahren', 'Lokale Produkte'], // Verknüpft mit einer Handlung, die den Fortschritt automatisch erhöht
    tips: [
  ...
  {
    id: 'preset-2',
    title: 'Einmal Unverpackt einkaufen',
    description: 'Kaufe einmal in einem Unverpackt-Laden ein',
    category: 'Shopping', 
    label:'Individual',
    target: 1,
    unit: 'Einkäufe',
  ...
  },
  {
    ...
    description: 'Erfasst 20 Handlungen im Bereich Ernährung',
    category: 'Ernährung',
    label:'Partner',
    target: 20,
    unit: 'Handlungen',
    linkedAction: 'Nachhaltige Ernährung', //hier noch mehr relevante Handlungen verknüpfen
    tips: [
      'Kaufe mehr pflanzliche Lebensmittel',
      'Vermeide Lebensmittelverschwendung',
      'Nutze saisonale Produkte',
      'Kaufe plastikfrei ein'
    ],
    type: 'preset'
  },
  {
    id: 'preset-4',
    title: 'Macher*in',
    description: 'Erreiche 100 Punkte an einem Tag',
    category: 'Allgemein',
    label:'Individual',
    target: 100,
    current: 100,
    unit: 'Punkte',
    linkedAction: 'Recycling', //Punkte durch verschiedene Handlungen sammeln
    tips: [
  ...
  {
    id: 'preset-5',
    title: '100 Tage nachhaltig handeln',
    description: 'Trage 100 Tage lang mindestens eine nachhaltige Handlung in deinem Alltag ein',
    category: 'Allgemein',
    label:'Individual',
    target: 100,
    unit: 'Tage',
    linkedAction: 'Lokale Produkte', //alle Handlungen können hier Fortschritt bringen
    tips: [
      'Fang mit kleinen Handlungen an',
      'Betrachte verschiedene Bereiche (Energie, Ernährung, Mobilität)',
      'Verknüpfe Handlungen mit deinen Interessen',
      'Gib nicht auf! Jeder Beitrag zählt'
    ],
    type: 'preset'
  }
];
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        // Füge preset-4 als abgeschlossenes Ziel hinzu, falls noch nicht vorhanden
        if (!parsed.some(g => g.id === 'preset-4')) {
          const preset4 = PRESET_GOALS.find(g => g.id === 'preset-4');
          if (preset4) {
            setActiveGoals(prev => [...prev, { ...preset4, current: preset4.target, completed: true }]);
          }
        }
      } catch (err) {
      ...
      // Kein gespeicherter Stand - füge preset-4 als abgeschlossenes Ziel hinzu
      const preset4 = PRESET_GOALS.find(g => g.id === 'preset-4');
      if (preset4) {
        setActiveGoals([{ ...preset4, current: preset4.target, completed: true }]);
      }
    }
  }, []);
