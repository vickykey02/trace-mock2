...
export const NotificationsProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    pendingActions.forEach(action => {
      ...
      const now = Date.now();
      const approvalDelay = 3 * 60 * 1000; // 10 * 1000 = 10 Sekunden zum Testen (später: 3 * 60 * 1000 für 3 Minuten)
      const timeElapsed = now - submittedTime;
      const timeRemaining = Math.max(approvalDelay - timeElapsed, 100); // Mindestens 100ms warten

      // Timer setzen für verbleibende Zeit (oder 100ms wenn Zeit bereits abgelaufen)
      const timer = setTimeout(() => {
        // Status auf 'approved' setzen
        updateStatus(action.id, 'approved');
        
        // Punkte dem Konto gutschreiben
        if (action.points > 0) {
          addPoints(action.points, `Handlung: ${action.actionName || action.category}`);
        }
        ...
        setNotifications(prev => [{
          ...
          time: timeRemaining > 100 ? 'Gerade eben' : 'Vor kurzem',
          read: false
        }, ...prev]);
  ...
  }, [submitted, processedActions, updateStatus, addPoints]);
