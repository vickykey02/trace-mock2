...
const Esse = ({ width = 60, height = 200 }) => {
  const { points, getFillPercent } = usePoints();
  
  // Füllstand aus dem globalen Punktestand
  const fillPercent = getFillPercent();
  
  // Die 7 Farbsegmente der Esse (von unten nach oben)
  const segments = [
	{ color: '#2596be', height: 8 },  // hellblau Fuß
    { color: '#fd7095', height: 16 },  // Rot
    { color: '#6ee491', height: 16 },  // Grün
  ...
  const kopfWidth = 12; // Breite des Kopfes
